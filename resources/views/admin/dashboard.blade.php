...
<div class="container mt-5">
    ...
    <div class="row">
        <div class="col-md-10 offset-md-1">
            <div class="card">
                <div class="card-header">Siswa Lunas</div>
                <div class="card-body">
                    <canvas id="canvas-status" height="200" width="500"></canvas>
                </div>
            </div>
        </div>
    </div>
    <div class="row">
        <div class="col-md-6">
            <div class="card">
                <div class="card-header">Siswa Lunas Pie Chart</div>
                <div class="card-body">
                    <canvas id="canvas-status-pie" height="200" width="500"></canvas>
                </div>
            </div>
        </div>
        <div class="col-md-6">
          <div class="card">
            <div class="card-body">
              <h4 >Keterangan</h4>
              <h5>Pembayaran SPP Tahun {{$tahun}}</h5>
              <p>Jumlah Siswa : {{$total_siswa}}</p>
              <p>Jumlah Siswa Lunas : {{$lunas[0]}}</p>
              <p>Jumlah Siswa Belum Lunas : {{$total_siswa - $lunas[0]}}</p>
              <p>Jumlah Siswa : {{$total_siswa}}</p>
...
var ctx = document.getElementById("canvas").getContext('2d');
  ...
  var statusChart = new Chart(document.getElementById("canvas-status").getContext('2d'), {
    type: 'line',
    data: {
      labels: {!!Universe::bulanAll()->map(fn($bulan)=>$bulan['nama_bulan'])!!},
      datasets: [
        {
          label: 'Tahun {{$tahun}}',
          data: {{$lunas}},
  ...
  var statusPieChart = new Chart(document.getElementById("canvas-status-pie").getContext("2d"),
    {
      type: 'pie',
      data: {
        labels: [
        ...
        datasets: [{
          label: '',
          data: [{{$lunas[0]}}, {{($total_siswa-$lunas[0])}}],
          backgroundColor: [
            'rgb(54, 162, 235)',
            'rgb(255, 205, 86)'
          ],
